Shader "LyumaShader/NewShaderSkin/NewSimpleSkinnedMeshOldUV34" {
	Properties {
	...
	SubShader {
		...
		#include "NewShaderSkinOldUV34.cginc"
...
void positionModClassic(inout float4 inVertex, inout float3 inNormal, inout float4 inTangent, inout float2 uv, float4 color, float _Frilling) {
    ...
    if (_Glossiness > 0.015 && _Glossiness < 0.025) {
        // astolfo mode
        if (uv.x < 0.125) {
            uv.y += 0.25;
            uv.x = 0.15;
        } else if (uv.x < 0.25 && uv.y < 0.25) {
            uv.y += 0.25;
            uv.x *= 2;
        }
        uv.x = 0.9 - 0.5 * uv.x;
        skirtHeight = saturate(0.25 - uv.y);
    }
    if (_Glossiness > 0.025 && _Glossiness < 0.035 && uv.y < 0.8) {
        // kotori mode
        if (uv.x > 0.5 && uv.x > 0.75) {
            uv.x = 0.75 - (uv.x - 0.75);//(uv.x - 0.5);
        }
        if (uv.x > 0.5) {
            uv.x = 0.42 + (0.5 * uv.x);
        }
        float centerdist = (uv.x - 0.7325) * (uv.x - 0.7325);
        uv.y -= 0.2 + 8 * centerdist;
        uv.y *= 0.5;
        /*
        uv.y += 0.05;
        skirtHeight = saturate(0.26 - uv.y);
        skirtHeight = 6 * skirtHeight * skirtHeight;
        */
        skirtHeight = saturate(0.25 - uv.y);
    }
    if (any(uv < .5) && (uv.x > 0.02 || uv.y < 0.98)) {
        ...
        if ((uv.x >0.75 || uv.y >0.25) && (_Glossiness < 0.015 && _Glossiness > 0.005)) {
            return;
        }
        if (uv.x <0.75 && _Glossiness > 0.015 && _Glossiness < 0.025) {
            return;
        }
