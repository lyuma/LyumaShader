Shader "LyumaShader/NewShaderSkin/NewSkinSilly_2d" {
	Properties {
...
float outmul = 2.5;
    ...
    if (_misete_coef < 0) {
        float ndotv = saturate(dot(normalApprox, cameraVec));
        {
            ...
            float lerpFactor2 = lerp(saturate(1 - cameraVec3d.y), cameraVec.y * cameraVec.y * abs(cameraVec3d.y), 0);
if (_Frilling) {
            EXT = 0.075;
} else {
            EXT = 0.03;
        ...
        float lerpFactor2 = lerp(1.0, cameraVec.y * cameraVec.y * cameraVec.y * cameraVec.y, 0);
