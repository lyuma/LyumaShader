...
Shader "LyumaShader/DropShadowLiteToonTransparent 1"
{
    Properties
    {
...
v2f_surf vertx (VertexInput v, float flipx) {
    ...
    actualObjectPos += float4((_shadow_offset.z * norm + flipx * _shadow_offset.x * tang + _shadow_offset.y * bitang),0.);// * dot(approxEyeDir.xz, cameraToObj2D.xy), 0.);
...
[maxvertexcount(6)]
void geom(triangle VertexInput IN[3], inout TriangleStream<v2f_surf> tristream)
{
    int i;
    //fixed facing = -1.*sign(sign(IN[0].pos.w) * cross(IN[2].pos.xyz - IN[1].pos.xyz, IN[1].pos.xyz - IN[0].pos.xyz).z);
    [unroll]
    for (i = 0; i < 3; i++) {
        //IN[i].tSpace0.xyz *= fixed3(facing, 1,facing);
        //IN[i].tSpace1.xyz *= fixed3(facing, 1,facing);
        //IN[i].tSpace2.xyz *= fixed3(facing, 1,facing);
        tristream.Append(vertx(IN[i], 1.));
    }
    for (i = 0; i < 3; i++) {
        tristream.Append(vertx(IN[i], -1.));
    }
